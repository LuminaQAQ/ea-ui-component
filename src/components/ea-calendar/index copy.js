...
export class EaCalendar extends Base {
  ...
  constructor() {
    ...
    this.#headerDateContent = shadowRoot.querySelector(
      ".ea-calendar-header_content"
    );
    this.#headerDateWrap = shadowRoot.querySelector(".ea-calendar-header_wrap");

    this.#headerChangerWrap = shadowRoot.querySelector(
      ".ea-calendar-header_changer"
    );
    this.#lastMonthBtn = shadowRoot.querySelector(
      ".ea-calendar-header_changer-lastMonth"
    );
    this.#todayBtn = shadowRoot.querySelector(
      ".ea-calendar-header_changer-today"
    );
    this.#nextMonthBtn = shadowRoot.querySelector(
      ".ea-calendar-header_changer-nextMonth"
    );
  ...
  set weekStart(weekStart) {
    ...
    this.#tableHead.innerHTML = createThead(
      getUserWeekStart(this.week, weekStart)
    ).innerHTML;
  }
  // #endregion
  // ------- end -------
  ...
  set date(date) {
    ...
    this.#headerDateContent.innerHTML = date = isNaN(new Date(date))
      ? getToday()
      : getUserToday(date);
  ...
  set size(size) {
    ...
    if (size === "mini") {
      ...
      this.#headerDateWrap.insertBefore(
        prevIcon,
        this.#headerDateWrap.firstChild
      );
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        if (
          ...
          lastDate.setDate(
            lastMonthDateStep > 0 ? day + length - step : lastMonthDateStep
          );
        ...
        if (
          ...
          currentDate.getDate() === myDate.getDate() + 1
        ) {
        ...
        if (
          ...
          currentDate.getDate() === userDate.getDate() + 1
        ) {
