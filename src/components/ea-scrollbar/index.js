...
export class EaScrollbar extends Base {
  ...
  $render() {
    ...
    this.#verticalTrack = this.shadowRoot.querySelector(
      ".ea-scrollbar__track-vertical"
    );
    this.#horizontalTrack = this.shadowRoot.querySelector(
      ".ea-scrollbar__track-horizontal"
    );
    this.#verticalThumb = this.shadowRoot.querySelector(
      ".ea-scrollbar__thumb-vertical"
    );
    this.#horizontalThumb = this.shadowRoot.querySelector(
   ...
   */
  #scrollEvent = () => {
    this.#verticalThumb.style.setProperty(
      "--ea-scrollbar-top",
      `${(this.#view.scrollTop / this.#view.scrollHeight) * 100}%`
    );
    this.#horizontalThumb.style.setProperty(
      "--ea-scrollbar-left",
      `${(this.#view.scrollLeft / this.#view.scrollWidth) * 100}%`
    );
  ...
  #resizeEvent = () => {
    queueMicrotask(() => {
      ...
      this.#verticalTrack.classList.toggle(
        "is-show",
        verticalThumbHeight >= 0.999
      );
      this.#horizontalTrack.classList.toggle(
        "is-show",
  ...
  #verticalMouseMoveEvent = e => {
    ...
    const scrollTop =
      scrollRatio * (this.#view.scrollHeight - this.#view.clientHeight);
  ...
  #horizontalMouseMoveEvent = e => {
    ...
    const scrollLeft =
      scrollRatio * (this.#view.scrollWidth - this.#view.clientWidth);
  ...
  #mouseDownEvent = e => {
    ...
    window.addEventListener(
      "mousemove",
      isHorizontal
        ? this.#horizontalMouseMoveEvent
        : this.#verticalMouseMoveEvent,
  ...
  #mouseKeyDownEvent = e => {
    if (e.key === "ArrowUp" || e.key === "ArrowDown") {
      this.#view.scrollTo({
        top:
          this.#view.scrollTop +
          (this.#view.scrollHeight / 8) * (e.key === "ArrowUp" ? -1 : 1),
        behavior: "smooth",
      });
      ...
      this.#view.scrollTo({
        left:
          ...
          (this.#view.scrollWidth / 8) * (e.key === "ArrowLeft" ? -1 : 1),
        behavior: "smooth",
      });
    }
  };
  ...
  connectedCallback() {
    ...
    this.#resizeEvent();
    this.#view.addEventListener("scroll", this.#scrollEvent, {
      signal: controller.signal,
    });
    this.#horizontalThumb.addEventListener("mousedown", this.#mouseDownEvent, {
      signal: controller.signal,
    });
    this.#verticalThumb.addEventListener("mousedown", this.#mouseDownEvent, {
      signal: controller.signal,
    });
    ...
    this.shadowRoot.addEventListener("slotchange", this.#resizeEvent, {
      signal: controller.signal,
    });
