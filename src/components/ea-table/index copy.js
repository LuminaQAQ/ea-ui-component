...
export class EaTable extends Base {
  ...
  constructor() {
    ...
    this.#headerTableColgroup = this.#headerTable.querySelector(
      ".ea-table_header colgroup"
    );
    this.#headerTableThead = this.#headerTable.querySelector(
      ".ea-table_header thead"
    );

    this.#bodyTable = this.shadowRoot.querySelector(".ea-table_main");
    this.#bodyTableColgroup = this.#bodyTable.querySelector(
      ".ea-table_body-wrap colgroup"
    );
    this.#bodyTableTbody = this.#bodyTable.querySelector(
      ".ea-table_body-wrap tbody"
    );
  ...
  get currentRowDetail() {
    ...
    const target =
      this.#bodyTable.querySelectorAll(".ea-table__row")[index] || null;
  ...
  #handleHasGutterTable() {
    ...
    setTimeout(() => {
      scrollbarWidth =
        ...
        this.#bodyTable.getBoundingClientRect().width;
      resizeFn();
    }, 0);
      ...
      Array.from(child).forEach((subchild) => {
        ...
        if (subchild.children.length > 0) {
          ...
          headerCol.setAttribute(
            "width",
            subchild.getAttribute("width") || 100
          );
      ...
      return data.map((item, index) => {
        ...
        arr.forEach((key, i) => {
          if (key === "index") obj[key] = index + 1;
            ...
            obj[key] = `<ea-checkbox></ea-checkbox>`;
  ...
  renderTableBody(data) {
    ...
    if (!this.#tableDataIsInit) {
      ...
      const thSequence = Array.from(
        this.#headerTable.querySelectorAll("ea-table-column")
      ).map((item, i) => {
      ...
      Object.entries(item).forEach(([key, value]) => {
        ...
        if (key === "selection") {
          cell.querySelector("ea-checkbox").addEventListener("change", (e) => {
            ...
            const checkedElements = Array.from(checkboxes).filter(
              (checkbox, index) => {
  ...
  #initSelectionTypeTh() {
    ...
    this.addEventListener("body-selection-change", (e) => {
      const checkallBtn = this.#headerTableThead
        .querySelector("ea-table-column")
        .shadowRoot.querySelector("ea-checkbox");
      const checkboxes = this.#bodyTable.querySelectorAll("ea-checkbox");
  ...
  #initHeaderSlot() {
    ...
    if (headerSlot.assignedNodes().length > 0) {
      ...
      th.part = "th-cell";
      let maxRowSpan = 1;
      Array.from(this.#headerTableThead.querySelectorAll("th")).forEach(
        (item) => {
