...
export class EaSelect extends Base {
  ...
  constructor() {
    ...
    this.#dropdownIcon = this.shadowRoot.querySelector(
      ".ea-select_dropdown-icon"
    );
    this.#dropdownWrap = this.shadowRoot.querySelector(
      ".ea-select_dropdown-wrap"
    );
    this.#noResultWrap = this.shadowRoot.querySelector(
      ".ea-select_dropdown-empty"
    );
    ...
    if (val) {
      this.#selectInput.addEventListener("change", e => {
        ...
        options.forEach(option => {
          option.style.display = option.value.includes(value)
            ? "block"
            : "none";
        });
