...
export class EaSplitter extends Base {
  ...
  #splitterColResizeEvent = e => {
    ...
    const mousemoveHandler = moveE => {
      ...
      if (preChild.min.endsWith("%")) {
        currentPreWidth =
          this.#container.clientWidth * parseCSSMinValue(preChild.min);
        currentNextWidth =
          this.#container.clientWidth * parseCSSMinValue(nextChild.min);
  ...
  #splitterRowResizeEvent = e => {
    ...
    const mousemoveHandler = moveE => {
      ...
      if (preChild.min.endsWith("%")) {
        currentPreHeight =
          this.#container.clientHeight * parseCSSMinValue(preChild.min);
        currentNextHeight =
          this.#container.clientHeight * parseCSSMinValue(nextChild.min);
      } else if (preChild.min.endsWith("px")) {
        ...
        if (child.tagName === "EA-SPLITTER-BAR") {
          ...
          child.addEventListener(
            ...
            this.layout === "horizontal"
              ? this.#splitterColResizeEvent
              : this.#splitterRowResizeEvent,
