...
.ea-checkbox-group ::slotted(ea-checkbox) {
  margin-right: 1.5rem;
}`;class P extends p{constructor(){super();const t=this.attachShadow({mode:"open"}),e=document.createElement("div");t.appendChild(e);const r=document.createElement("slot");e.className="ea-checkbox-group",e.appendChild(r),this.dom=e;const i=document.createElement("style");i.innerHTML=q,this.shadowRoot.appendChild(i),t.appendChild(e)}get name(){return this.getAttribute("name")}set name(t){this.querySelectorAll("ea-checkbox").forEach(e=>{e.setAttribute("name",t),e.name=t})}get value(){if(this.getAttribute("value"))return this.getAttribute("value")}set value(t){if(t)try{t.split(",").map(r=>r.trimStart()).map(r=>{const i=this.querySelector(`ea-checkbox[value="${r}"]`);i.checked=t})}catch{}}get disabled(){return this.getAttrBoolean("disabled")}set disabled(t){this.querySelectorAll(`ea-checkbox[value="${this.name}"]`).forEach(r=>{r.disabled=t})}init(){this.name=this.name,this.value=this.value,this.disabled=this.disabled}connectedCallback(){this.init()}}const l=(o,t)=>{window.customElements.get(o)||window.customElements.define(o,t)};l("ea-button",k);l("ea-button-group",w);l("ea-link",z);l("ea-radio",C);l("ea-radio-group",N);l("ea-checkbox",R);l("ea-checkbox-group",P);
