...
class m extends p {
  ...
  // #region
  get size() {
    const a = this.getAttrNumber("size"), s = this.getAttribute("size");
    return a === 0 || !a ? ["large", "medium", "small"].includes(s) ? s : "normal" : this.getAttrNumber("size");
  }
  set size(a) {
    this.setAttribute("size", a), typeof a == "number" ? (r(this, e).style.width = `${a}px`, r(this, e).style.height = `${a}px`, r(this, e).style.lineHeight = `${a}px`) : typeof a == "string" && r(this, e).classList.add(`ea-avatar--${a}`);
  }
  // #endregion
  // ------- end -------
  // ------- shape 图片形状 -------
  // #region
  get shape() {
    const a = this.getAttribute("shape");
    return ["circle", "square"].includes(a) ? a : "circle";
  }
  set shape(a) {
    this.setAttribute("shape", a), r(this, e).classList.add(`ea-avatar--${this.shape}`);
  }
  // #endregion
  // ------- end -------
  // ------- src 图片链接 -------
  ...
  set fit(a) {
    this.setAttribute("fit", a), r(this, e).classList.add(`ea-avatar-fill--${a}`);
  }
  // #endregion
  // ------- end -------
  connectedCallback() {
