...
var c = (a, t, e) => ($(a, t, "read from private field"), e ? e.call(a) : t.get(a)), l = (a, t, e) => {
  if (t.has(a))
    throw TypeError("Cannot add the same private member more than once");
  t instanceof WeakSet ? t.add(a) : t.set(a, e);
}, i = (a, t, e, r) => ($(a, t, "write to private field"), r ? r.call(a, e) : t.set(a, e), e);
var D = (a, t, e) => ($(a, t, "access private method"), e);
import { B as J } from "./Base.js";
import { c as o } from "./createElement.js";
import "./ea-button-group.js";
import "./ea-button.js";
const K = (a = ["一", "二", "三", "四", "五", "六", "日"]) => {
  const t = o("tr");
...
`;
var y, h, x, v, b, m, g, M, w, T, A, L, z, k, W;
class Q extends J {
  constructor() {
    super();
    /**
  ...
  set weekStart(e) {
    this.setAttribute("week-start", e), c(this, M).innerHTML = K(j(this.week, e)).innerHTML;
  }
  // #endregion
  // ------- end -------
  // ------- date 用户传入日期 -------
  // #region
  get date() {
    return this.getAttribute("date") || I();
  }
  set date(e) {
    this.setAttribute("date", e), c(this, x).innerHTML = e = isNaN(new Date(e)) ? I() : O(e), D(this, k, W).call(this, c(this, w), e, this.weekStart);
  }
  // #endregion
  // ------- end -------
  // ------- size 用户传入尺寸 -------
  ...
  set size(e) {
    if (this.setAttribute("size", e), c(this, y).classList.add(e), e === "mini") {
      const r = o("span", "prev-btn");
      r.innerText = "<";
      const d = o("span", "next-btn");
      d.innerText = ">", i(this, b, r), i(this, g, d), c(this, h).insertBefore(r, c(this, h).firstChild), c(this, h).appendChild(d);
    }
  }
  // #endregion
  // ------- end -------
  ...
  connectedCallback() {
    this.weekStart = this.weekStart, this.date = this.date, this.size = this.size, c(this, b).addEventListener("click", () => {
      D(this, L, z).call(this, "last");
    }), c(this, m).addEventListener("click", () => {
      this.date = `${(/* @__PURE__ */ new Date()).getFullYear()}-${(/* @__PURE__ */ new Date()).getMonth() + 1}-${(/* @__PURE__ */ new Date()).getDate()}`;
    }), c(this, g).addEventListener("click", () => {
      D(this, L, z).call(this, "next");
    });
  }
}
y = new WeakMap(), h = new WeakMap(), x = new WeakMap(), v = new WeakMap(), b = new WeakMap(), m = new WeakMap(), g = new WeakMap(), M = new WeakMap(), w = new WeakMap(), T = new WeakSet(), A = function(e) {
  e.addEventListener("click", (r) => {
    c(this, w).querySelectorAll("td").forEach((p) => {
...
}, k = new WeakSet(), W = function(e, r, d = "一") {
  r = isNaN(new Date(r)) ? /* @__PURE__ */ new Date() : new Date(r), e.innerHTML = "";
  const p = new Date(r), _ = p.getMonth() + 1, n = new Date(r);
  n.setDate(1);
  const u = new Date(r);
  u.setMonth(_), u.setDate(0);
  const F = new Date(r);
  F.setMonth(_), F.setDate(1);
  const Y = j(this.week, d);
  for (let B = 0; B < 6; B++) {
    const S = o("tr");
    S.part = "table-body-row";
    for (let C = 0; C < 7; C++) {
      const { length: H } = S.children, s = o("td");
      s.part = "table-body-cell";
      const f = o("span");
      f.part = "table-body-cell-content";
      const E = n.getDay(), q = /* @__PURE__ */ new Date();
      if (Y[H] === this.week[E] && _ === n.getMonth() + 1)
        f.innerText = n.getDate(), n.setDate(n.getDate() + 1), D(this, T, A).call(this, s);
      else if (_ == n.getMonth())
        f.innerText = n.getDate(), n.setDate(n.getDate() + 1), s.classList.add("is-disabled"), s.part = "table-body-cell-disabled";
      else {
        const N = C - E + 2, R = Y.findIndex((U, G) => {
          if (U === "一")
            return G;
        });
        u.setMonth(_ - 1), u.setDate(N > 0 ? E + H - R : N), f.innerText = u.getDate(), s.part = "table-body-cell-disabled", s.classList.add("is-disabled");
      }
      new Date(this.date), n.getFullYear() === q.getFullYear() && n.getMonth() === q.getMonth() && n.getDate() === q.getDate() + 1 && (s.part = "table-body-cell-today", s.classList.add("is-today")), n.getFullYear() === p.getFullYear() && n.getMonth() === p.getMonth() && n.getDate() === p.getDate() + 1 && (s.part = "table-body-cell-selected", s.classList.add("is-selected")), s.appendChild(f), S.appendChild(s);
    }
    e.appendChild(S);
  }
};
customElements.get("ea-calendar") || customElements.define("ea-calendar", Q);
export {
  Q as EaCalendar
};
